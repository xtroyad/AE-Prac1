...
# Calculo la matriz de correlación de las columnas numéricas y la visualizo
...
```
```{r}
# Identificación variables con alta correlación
findCorrelation(cor(df[, c("lcavol", "lweight", "lbph", "lcp", "svi")]), 
                names = TRUE,
                cutoff = 0.44)
```
## Ajuste de la Regresión con Componentes Principales (PCR)
Para abordar la colinealidad, ajusto un modelo de PCR y uso validación cruzada para evaluar el error de predicción:

```{r}
set.seed(42)
pcr.fit <- pcr(lpsa ~ lcavol + lweight + lbph + lcp + svi, data = df, scale = TRUE, validation = "CV")
summary(pcr.fit)
```
...
Para decidir el número óptimo de componentes principales, visualizo el MSE con el siguiente gráfico:

```{r}
validationplot(pcr.fit, val.type = "MSEP")
```
Visualmente ya ve ve que  el menor error de validación cruzada más pequeño es con M = 5.
...
pcr.fit.train <- pcr(lpsa ~ lcavol + lweight + lbph + lcp + svi,, data = train_data, scale = TRUE, validation = "CV")
...
```{r}
ncomp_opt <- 5  # Ajusto este número basado en el gráfico de MSEP
pcr.pred <- predict(pcr.fit.train, test_data, ncomp = ncomp_opt)
...
```{r}
pcr.final <- pcr(lpsa ~ ., data = df, scale = TRUE, ncomp = ncomp_opt)
summary(pcr.final)
```
