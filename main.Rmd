...
```{r}
prostate$svif <- factor(prostate$svi, levels = c(0, 1), labels = c("No", "Yes"))
prostate$gleasonf <- factor(prostate$gleason)
```
...
barplot(table(prostate$svif), main = "Distribución de SVI", col = "lightblue")
barplot(table(prostate$gleasonf), main = "Distribución de Gleason", col = "lightgreen")
```
...
prostate$agef <- cut(prostate$age, breaks = seq(from=min.age, to=max.age+5, by=5), right=FALSE)
barplot(table(prostate$agef), main = "Distribución de Edad", col = "salmon", width = 3)
```
