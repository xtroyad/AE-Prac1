...
6.  **`pgg45_num`**:
    -   **Máxima correlación mutua:** `gleason_num` (0.752).
...
```{r}
# Se crea los conjuntos de entrenamiento y prueba
set.seed(1)
...
pcr.fit.train <- pcr(lpsa ~ lcavol + lweight + lbph + lcp + svi,, 
                     data = train_data,
                     scale = TRUE, 
                     validation = "CV")
...
Usemos `which.min()` para tenerlo más claro...:
```{r}
which.min( MSEP( pcr.fit.train )$val[1,1, ] ) - 1
```
Vemos que en este caso el mejor `M` es 2 para este conjunto de entrenamiento, y no tendriamos que tener en cuenta todas la variables.

##  Evaluación del modelo en el conjunto de prueba
Se predicen los valores de `lpsa` en el conjunto de prueba y calculo el `MSE` para medir la precisión del modelo:
```{r}
ncomp_opt <- 2
pcr.pred <- predict(pcr.fit.train, 
                    test_data, 
                    ncomp = ncomp_opt)

# Calculo el MSE
mse <- mean((pcr.pred - test_data$lpsa)^2)
print(mse)
...
```{r}
pcr.final <- pcr(lpsa ~ lcavol + lweight + lbph + lcp + svi, 
                 data = df, 
                 scale = TRUE, 
                 ncomp = 2)
summary(pcr.final)
...
# MSE pcr
mse_pcr_full <- mean((pcr.pred_full - df$lpsa)^2)
# MSE multi
mse_multi_full <- mean((multi_pred_full - df$lpsa)^2)
...
cat("PCR: regresión de componentes principales\n",
    ...
    "############ MSE ############\n\n",
    "MSE (PCR):", mse_pcr_full, "\n",
    "MSE (MULTI):", mse_multi_full, "\n\n",
    "############ R² ############\n\n",
...
Ambos modelos, **PCR** y **Regresión Lineal Múltiple (MULTI)**, tienen un MSE de **0.469**, lo que indica que la precisión de las predicciones en ambos casos es comparable.
...
Aunque ambos modelos presentan un MSE igual, lo que refleja una precisión similar en las predicciones, el modelo **PCR** tiene una ligera ventaja en cuanto a **R² ajustado**. Esto sugiere que **PCR** está mejor ajustado a los datos y puede estar aprovechando de manera más eficiente la estructura de las variables. Sin embargo, la diferencia no es tan grande, por lo que ambos modelos pueden ser igualmente válidos dependiendo del contexto y los requisitos del análisis.
